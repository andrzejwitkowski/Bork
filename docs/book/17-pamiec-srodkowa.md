...
Analiza własności potrafi powiedzieć, że nazwa została przeniesiona, i na tym poprzestać. Nie mówi jeszcze, w którym buforze powstanie napis ani czy jego pamięć pożyje dość długo, żeby funkcja mogła go zwrócić. Gdyby pominąć te pytania, program z poprawnym `move` i tak mógłby oddać napis, którego pamięć znika razem z blokiem albo razem z wołaną funkcją. Dlatego po czystym sprawdzeniu typów i własności kompilator zadaje jeszcze trzy pytania o miejsce, i dopiero ich odpowiedzi wolno uznać za koniec sprawdzenia.
...
- pytanie, czy pamięć zwracanego napisu pożyje po powrocie z funkcji,
...
Pytanie tej części brzmi, gdzie ma leżeć treść wartości, która nie jest kopią, i jak długo to miejsce wolno jej zajmować. Liczba całkowita nie ma tego problemu, bo mieści się w samej wartości i kopiuje się przy odczycie. Napis i tablica trzymają treść inaczej, bo leży ona w buforze regionu, a nazwa pamięta tylko opis, gdzie ta treść jest i jak jest długa. Gdy region się kończy, bufor jest zwalniany, i każdy opis, który wciąż na niego wskazuje, zostaje wskaźnikiem do zwolnionej pamięci, po angielsku dangling pointer. Odczyt przez taki wskaźnik nie jest już odczytem napisu, tylko odczytem pamięci, która do tego napisu nie należy. Własność z rozdziału 14 pilnuje, żeby dwóch właścicieli nie było naraz. Ten rozdział pilnuje, żeby jedyny właściciel nie przeżył swojego bufora.

Te trzy pytania padają tylko wtedy, gdy lista komunikatów po typach i własności jest pusta. Przy błędzie składni, typu albo przeniesienia kompilator nie wybiera miejsca na napis, bo i tak nie będzie generował kodu. Gdy pytania padną i któreś odpowie odmową, reprezentacja pośrednia znika z wyniku tak samo jak przy błędzie typu, a raport regionów zostaje. Komunikat takiej odmowy ma fazę `ownership`, nie osobną nazwę, bo z punktu widzenia wydruku to wciąż skarga o czas życia napisu, a nie o kształt tekstu.

<!-- figura: Rysunek 16.1. Kolejność trzech pytań o miejsce napisu w jednym przebiegu -->
```mermaid
flowchart TD
    ...
    miejsce --> powrot["Czy pamięć napisu pożyje po powrocie z funkcji"]
    powrot -->|nie| odmowa["Komunikat fazy ownership i kod 1"]
    powrot -->|tak| emisja["Wolno budować plik wykonywalny"]
```

Rysunek 16.1 jest kolejnością w jednym przebiegu, a nie trzema niezależnymi narzędziami. Wejście do regionu przy wykonaniu jest potrzebne tylko blokowi, który naprawdę coś alokuje. Blok, który jedynie czyta liczbę albo współdzieli napis zadeklarowany wyżej, nie dostaje własnego bufora, bo nie miałby czego w nim położyć. To jest pierwsza odpowiedź i ona nie zmienia tekstu programu. Zaznacza tylko, które węzły raportu mają przy generowaniu kodu wołać wejście i wyjście regionu.

## Gdzie powstaje napis, który zaraz zmienia właściciela

Pytanie brzmi, co zrobić z napisem, który w tekście stoi w bloku wewnętrznym, ale jego jedynym dalszym życiem jest nazwa z bloku zewnętrznego. Gdyby napis powstał w buforze wewnętrznym, koniec bloku zwolniłby go, zanim zewnętrzna nazwa skończyłaby z niego korzystać. Dałoby się to odrzucić, i część takich zapisów rzeczywiście jest odrzucana. Da się też zauważyć, że nazwa wewnętrzna jest tylko przystankiem, i położyć napis od razu w regionie nazwy, która go przejmuje. Kompilator robi to drugie wtedy, gdy układ instrukcji jest jednoznaczny.
...
Widać to na programie, który nadpisuje `outer` literałem z bloku i potem ten napis wypisuje. Plik `15-assign-up.bork` przechodzi sprawdzenie z kodem 0. Samo sprawdzenie nie wypisuje miejsca alokacji, bo wydruk regionów tego nie pokazuje, więc o tym, że napis przeżył blok, rozstrzyga dopiero uruchomienie. Poniższy tekst jest całym programem, bez skrótów.
...
Pozostałe odmowy tej kontroli są o tym samym. Przypisanie do nazwy wartości, której pamięć leży w regionie wewnętrznym, kończącym się wcześniej niż ta nazwa, jest odrzucane, gdy nie da się położyć jej od razu u celu. Gałąź `if`, która przenosi napis i sama jest wartością warunku, też nie może oddać napisu ze swojego regionu, bo ten region znika razem z gałęzią. Pełna lista takich miejsc jest w `check_function` w pliku `src/escape.rs`. Nie wnosi ona nowego pytania ponad to, czy opis napisu nie przeżywa bufora, w którym leży jego treść.
...
- Po czystych typach i własności kompilator pyta jeszcze, które bloki alokują, gdzie ma powstać napis przejmowany przez nazwę zewnętrzną i czy zwrot przeżyje powrót z funkcji.
...
- Zwrot wyniku `concat` oraz zwrot przez `move` są odrzucane, bo pamięć napisu nie przeżyłaby regionu wołanej funkcji, natomiast zwrot lokalnej nazwy albo literału jest dozwolony.
